...
var GameLayer = cc.Layer.extend({
    ...
    _isBackTileReload:false,//外星建筑
    lbScore:null,
    screenRect:null,
    explosionAnimation:[],//爆炸效果资源
    _beginPos:cc.p(0, 0),//开始位置
    ...
    init:function () {
        ...
        if (this._super()) {
            ...
            // schedule
            this.scheduleUpdate();//帧循环
            this.schedule(this.scoreCounter, 1);//每一秒更新一次分数
            //播放背景音
            if (MW.SOUND) {
            ...
            Bullet.preSet();//初始化子弹
            Enemy.preSet();//初始化敌人
            HitEffect.preSet();//初始化碰撞效果
            SparkEffect.preSet();//闪光效果
            Explosion.preSet();//爆炸效果
            BackSky.preSet();//图片背景
            BackTileMap.preSet();//tilemap背景
    ...
    update:function (dt) {
        if (this._state == STATE_PLAYING) {
            this.checkIsCollide(); //检测碰撞
            this.removeInactiveUnit(dt);//移除不活动的元素
            this.checkIsReborn();//检测生命值和再生
            this.updateUI();//检测并更新分数和生命值
            this._movingBackground(dt);//移动地图
        }
    },
    //碰撞检测
